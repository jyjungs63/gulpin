...
function parseBookParameter(bookParam) {
    ...
    if (parts.length === 2) {
        ...
        step      = "";
        vol       = "";
        directory = `${rootPath}${BOOK_CONFIG.DIRECTORY_MAP[type]}${parts[1]}/`;
    } else if (parts.length === 3) {
        // type.step.vol → 기존 동작
        step      = parts[1];
        vol       = parts[2];
        directory = `${rootPath}${BOOK_CONFIG.DIRECTORY_MAP[type]}${step}/${vol}/`;
    } else {
        // type.a.b.c... → 나머지 항목을 /로 결합
        step      = parts[1];
        vol       = "";
        const subPath = parts.slice(1).join('/');
        directory = `${rootPath}${BOOK_CONFIG.DIRECTORY_MAP[type]}${subPath}/`;
    }
