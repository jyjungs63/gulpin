...
require_once 'sqlite_compat.php';
...
function CheckPasswd($login, $password)
{
    global $conn;
    $user = "";
    $login = sqlite_escape_string($conn, $login);
    $rs = sqlite_query($conn, "select * from chaitalk_user where id='{$login}' ");

    if ($rs) {
        $user = sqlite_fetch_assoc($rs);
        $passwordnew = sqlite_escape_string($conn, password_hash($password, PASSWORD_BCRYPT));
        sqlite_free_result($rs);
    }
    $conn->close();
    return $user;
}
...
function CheckUser($login, $password)
{
    ...
    $user = "";
    $login = sqlite_escape_string($conn, $login);
    $rs = sqlite_query($conn, "select * from chaitalk_user where id='{$login}' and status = 1 and confirm = 1 ");

    if ($rs) {
        $user = sqlite_fetch_assoc($rs);
        $passwordnew = sqlite_escape_string($conn, password_hash($password, PASSWORD_BCRYPT));
        $storedPassword = $user && isset($user['password']) ? $user['password'] : '';
        $isHash = !empty(password_get_info($storedPassword)['algo']);
        if ($user &&  (($isHash && !password_verify($password, $storedPassword)) || (!$isHash && strcmp($password, $storedPassword) != 0))) {
            $user = "";
        }
        sqlite_free_result($rs);
    }
...
function SRegister($data, $dest)
{
    ...
    try {
        ...
        $id = sqlite_escape_string($conn, $id);
        ...
        $res = sqlite_query($conn, $sqlstring);
...
function SstudentList($data)
{
    ...
    try {
        ...
        $rs = sqlite_query($conn, $sqlString);
...
function SPorDetailListRangeSort($data)
{
    ...
    try {
        ...
        $rs1 = sqlite_query($conn, $stmt);
        $rows = [];
        while ($row = sqlite_fetch_array($rs1)) {
            array_push(
                $rows,
                array(
                    'id'     => $row['id'],
...
function SShowAddr($data)
{
    ...
    try {
        ...
        $rs = sqlite_query($conn, $sqlString2);
...
function SShowStudentList($data)
{
    ...
    if ($tid == "admin") {
        if ($sel == '1') {
            ...
            $sqlString = "SELECT *  FROM chaitalk_user au, (select id from chaitalk_user ) u where u.id = au.tid and role = 0 and classnm='{$step}'";
        } else if ($sel == '3')
...
function saveClientlog($id, $name, $role) {
    // require_once 'sqlite_compat.php'; // 필요 시 주석 해제
    global $conn;
