...
let bookInfo = null;
let pdfObjectUrls = [];
const PRINT_ENABLED = new URLSearchParams(window.location.search)
    .get("PRINT_ENABLED")?.toLowerCase() !== "false";
...
async function loadPdfDocument(pdfUrl) {
    if (typeof window.pdfjsLib === 'undefined') {
        throw new Error('PDF.js를 불러오지 못했습니다. 네트워크 연결을 확인하세요.');
    }

    window.pdfjsLib.GlobalWorkerOptions.workerSrc =
        'https://cdnjs.cloudflare.com/ajax/libs/pdf.js/3.11.174/pdf.worker.min.js';

    return await window.pdfjsLib.getDocument(pdfUrl).promise;
}

async function renderPdfPages(pdfDocument, pageIndices) {
    clearPdfObjectUrls();

    for (const pageNumber of pageIndices) {
        const pdfPage = await pdfDocument.getPage(pageNumber);
        const baseViewport = pdfPage.getViewport({ scale: 1 });
        const scale = Math.min(2, 1800 / baseViewport.width);
        ...
        const blob = await new Promise((resolve, reject) => {
            canvas.toBlob(value => value ? resolve(value) : reject(new Error('PDF 페이지 이미지 생성 실패')), 'image/png');
        });
...
async function init() {
    ...
    try {
        ...
        videoLinks = {};
        const urlParams = new URLSearchParams(window.location.search);
        const requestedPdf = (urlParams.get('pdf') || '').trim();

        if (requestedPdf && (!/^[^/\\]+\.pdf$/i.test(requestedPdf) || requestedPdf.includes('..'))) {
            throw new Error('pdf 파라미터에는 현재 책 폴더의 PDF 파일명만 사용할 수 있습니다.');
        }
        ...
        let numbook = Number(scanResult.image_count) || 0;
        let pdfDocument = null;
        const pdfFile = requestedPdf || (numbook === 0 ? scanResult.pdf_file : null);

        if (pdfFile) {
            const pdfUrl = `${imgpath}${encodeURIComponent(pdfFile)}`;
            console.log('[init] PDF 로딩:', pdfUrl);
            pdfDocument = await loadPdfDocument(pdfUrl);
            numbook = pdfDocument.numPages;
            imgfomat = 'pdf';
        }

        // 이미지와 PDF가 모두 없을 때 클라이언트 이미지 탐색으로 재시도
        if (numbook === 0) {
            ...
            if (fallback.image_count > 0) {
                ...
                const pdfHint = scanResult.pdf_count > 1
                    ? ' PDF가 여러 개이면 ?pdf=파일명.pdf로 선택해야 합니다.'
                    : '';
                throw new Error(`이미지 또는 PDF 파일을 찾을 수 없습니다. 경로: ${imgpath}.${pdfHint}`);
            }
        }
        ...
        if (pdfDocument) {
            await renderPdfPages(pdfDocument, pageIndices);
